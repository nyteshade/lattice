...
class SyntaxTree {
   ...
   */
  get schema() {
    return (0, _graphql.buildASTSchema)(this.ast);
  }
            ...
            // The kinds are the same, lets just merge their fields
                ...
                switch (theirs.kind) {
                  ...
                  case 'EnumTypeDefinition':
                    ...
                    for (let side of ours.values.concat(theirs.values)) {
                      if (!map.has(side.name.value)) {
                        map.set(side.name.value, side);
                      }
                    }

                    ours.values = (0, _from2.default)(map.values());
  ...
  consumeDefinition(astOrSyntaxTree, definitionType = "Query") {
    ...
    const tree = (0, _types.typeOf)(astOrSyntaxTree) === SyntaxTree.name ? astOrSyntaxTree : SyntaxTree.from(astOrSyntaxTree);
   ...
   * `SyntaxTree` it
   */
  consumeAllExecutableTypes(astOrSyntaxTree) {
    let types = [];
    let schema;

    if ((0, _types.typeOf)(astOrSyntaxTree) === SyntaxTree.name) {
  ...
  *[_iterator2.default]() {
   ...
   */
  static readType(typeNode, setName) {
    let result = {
      required: typeNode.kind === 'NonNullType',
      array: typeNode.kind === 'ListType' || typeNode.type && typeNode.type.kind === 'ListType' || false,
      type: typeNode.type && typeNode.type.type && typeNode.type.type.type && typeNode.type.type.type.name && typeNode.type.type.type.name.value || typeNode.type && typeNode.type.type && typeNode.type.type.name && typeNode.type.type.name.value || typeNode.type && typeNode.type.name && typeNode.type.name.value || typeNode.name && typeNode.name.value || undefined
    };
  ...
  get outline() {
    ...
    for (let definition of this) {
      ...
      switch (definition.kind) {
        ...
        case 'ObjectTypeDefinition':
          let key = 'InterfaceTypeDefinition' === definition.kind ? 'interfaces' : 'ObjectTypeDefinition' === definition.kind ? 'types' : 'inputs';
          ...
          definition.fields.forEach(field => {
            ...
            if (field.arguments) {
              out[name].args = field.arguments.map(arg => {
                return {
                  [arg.name.value]: SyntaxTree.readType(arg.type, arg.name.value)
                };
              });
            } else {
              out[name].args = [];
  ...
  */
   ...
   */
  static fromSDL(sdl) {
    const ast = (0, _graphql.parse)(sdl);
